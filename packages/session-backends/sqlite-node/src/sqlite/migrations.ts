...
/**
 * The initial schema, inlined from `./migrations/001_initial.sql` so the
 * backend can run where there is no filesystem to read it from (browsers,
 * Workers). The `.sql` file stays the human-readable source; keep the two in
 * step.
 */
export const INITIAL_SCHEMA_SQL = `-- AgentHarness storage format 4 / storageVersion 1.
